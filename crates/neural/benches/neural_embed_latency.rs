...
//
// §12.3 required benchmark: EmbedEngine::embed latency per batch size.
//
// Models are expected to be present under data/models/. Run raithe.sh
// from the workspace root before benchmarking.
...
/// Benchmarks `EmbedEngine::embed` across increasing batch sizes.
///
...
fn bench_embed_latency(c: &mut Criterion) {
    ...
    let mut engine = EmbedEngine::new(&NeuralConfig::default(), Arc::clone(&metrics))
        .expect("internal error: EmbedEngine init failed — models must be present");
